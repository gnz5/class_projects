...
# The maximum single packet needs to have 40 bytes reserved for encryption overhead
MAX_CHUNK_SIZE = MAXIMUM_PAYLOAD_SIZE - 40
...
class socket:
    ...
    def __init__(self):
        ...
        # Whether this socket sends encrypted packets or not
        self.encrypt = False
    ...
    def connect(self, address, encrypt = 0):
    ...
    def create_data_packets(self, buffer):
        chunks = [buffer[i:i + MAX_CHUNK_SIZE] for i in range(0, len(buffer), MAX_CHUNK_SIZE)]

        # iterates up to total packets and creates each packet
        for payload in chunks:
            # If we're encrypting, set the options and encrypt the payload
            if self.encrypt:
                flags = SOCK352_HAS_OPT
                option = 1
                payload = self.box.encrypt(payload)
            else:
                flags = 0x0
                option = 0
            # creates the new packet with the appropriate header
            new_packet = self.createPacket(flags=flags,
                                           sequence_no=self.sequence_no,
                                           ack_no=self.ack_no,
    ...
    def createPacket(self, flags=0x0, sequence_no=0x0, ack_no=0x0, payload_len=0x0, option=0x0):
    ...
    def manage_recvd_data_packet(self, packet):
        ...
        if self.encrypt and packet_header[PACKET_FLAG_INDEX] & SOCK352_HAS_OPT != 0 and \
                packet_header[PACKET_OPT_INDEX] == 1:
