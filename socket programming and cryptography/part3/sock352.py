...
MAX_WINDOW = 32 * 1024 / MAXIMUM_PAYLOAD_SIZE
...
class socket:
    ...
    def __init__(self):
        ...
        # Only used for the sender
        # Stores the window size of the receiver
        self.current_window_size = 0

        # Only used for the sender
        # Store the current congestion control window size
        self.congestion_control = 2

        # The lock for congestion control to avoid race conditions
        self.congestion_lock = threading.Lock()
    ...
    def connect(self, address):
        ...
        self.current_window_size = syn_ack_packet[PACKET_WINDOW_INDEX]
    ...
    def send(self, buffer):
        ...
        while not self.can_close:
            ...
            self.congestion_lock.acquire()
            congestion_remaining = self.congestion_control
            self.congestion_lock.release()

            window_remaining = self.current_window_size
            sent = 0
            ...
            # Additionally, do not send more packets than the receiver can currently take
            # Also obey congestion control
            while not self.can_close and resend_start_index < total_packets and not self.retransmit and window_remaining > 0 and congestion_remaining > 0:
    ...
    def recv_acks(self):
        ...
        while not self.can_close:
            ...
            try:
                ...
                self.current_window_size = new_packet[PACKET_WINDOW_INDEX]
    ...
    def recv(self, nbytes):
        ...
        self.send_ack(MAX_WINDOW)

        # keep trying to receive packets until the receiver has more bytes left to receive
        while bytes_to_receive > 0 and (len(data_received) + MAXIMUM_PAYLOAD_SIZE) // MAXIMUM_PAYLOAD_SIZE <= MAX_WINDOW:
            ...
            try:
                ...
                packet_received = self.socket.recv(MAXIMUM_PACKET_SIZE)

                # sends the packet to another method to manage it and gets back the data in return
                str_received = self.manage_recvd_data_packet(packet_received, MAX_WINDOW - len(data_received) // MAXIMUM_PAYLOAD_SIZE)
        ...
        print ("Received %d bytes of data" % len(data_received))
    ...
    def createPacket(self, flags=0x0, sequence_no=0x0, ack_no=0x0, payload_len=0x0, window_size=0x0):
    ...
    def manage_recvd_data_packet(self, packet, window_size):
